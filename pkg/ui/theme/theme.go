...
// StatusBar renders a persistent status bar with view context, key hints, and gradient accent.
func (t *Theme) StatusBar(view string, hints []string, width int) string {
	...
	// Gradient accent
	accent := t.GradientBar(max(10, width-len(viewText)-len(hintsText)-2))

	return "\n" + lipgloss.JoinHorizontal(lipgloss.Top, viewText, hintsText, accent)
}
